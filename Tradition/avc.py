...
def avc_compress_hsi(hsi_path, output_dir, crf_levels, device, save_mat):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for crf in crf_levels:
            print(f"\nAVC - 目标 CRF: {crf}")
            video_path = os.path.join(output_dir, f"{Path(hsi_path).stem}_avc_crf{crf}.mp4")
            ...
            if save_mat:
                output_mat = os.path.join(output_dir, f"{Path(hsi_path).stem}_avc_crf{crf}.mat")
                sio.savemat(output_mat, {'data': rec_hsi_data.transpose(1, 2, 0)})
        
        image_name = Path(hsi_path).stem
        os.makedirs(output_dir, exist_ok=True)
        json_path = os.path.join(output_dir, f"{image_name}_avc_results.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--crf_levels', type=str, default='18,23,28,33', help='逗号分隔的AVC CRF值列表 (更低的值=更高质量)')
